...
func TestReserveJtaUtxoChain(t *testing.T) {
	...
	for i, c := range cases {
		...
		utxos, err := m.reserveJtaUtxoChain(&txbuilder.TemplateBuilder{}, "TestAccountID", c.amount, false)
...
func TestBuildJtaTxChain(t *testing.T) {
	txbuilder.ChainTxUtxoNum = 3
	m := mockAccountManager(t)
	cases := []struct {
		inputUtxo  []uint64
	...
	for caseIndex, c := range cases {
		...
		tpls, gotUtxo, err := m.buildJtaTxChain(utxos, acct.Signer)
		if err != nil {
			t.Fatal(err)
		}
...
func TestMergeSpendAction(t *testing.T) {
	testJTA := &bc.AssetID{}
	if err := testJTA.UnmarshalText([]byte("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff")); err != nil {
		t.Fatal(err)
	}
