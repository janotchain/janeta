...
import (
	...
	"github.com/janotchain/janeta/math/checked"
...
func checkValid(vs *validationState, e bc.Entry) (err error) {
	...
	case *bc.Mux:
		...
		for assetID, amount := range parity {
			if assetID == *consensus.JTAAssetID {
	...
	case *bc.VoteOutput:
		...
		if *e.Source.Value.AssetId != *consensus.JTAAssetID {
	...
	case *bc.Coinbase:
		...
		if *e.WitnessDestination.Value.AssetId != *consensus.JTAAssetID {
