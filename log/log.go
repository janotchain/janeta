...
func newJtaHook(logPath string) *JtaHook {
	hook := &JtaHook{lock: new(sync.Mutex)}
	hook.logPath = logPath
	return hook
}

// Write a log line to an io.Writer.
func (hook *JtaHook) ioWrite(entry *logrus.Entry) error {
	module := "general"
...
func (hook *JtaHook) Fire(entry *logrus.Entry) error {
	hook.lock.Lock()
...
func (hook *JtaHook) Levels() []logrus.Level {
