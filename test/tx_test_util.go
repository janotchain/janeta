...
func CreateCoinbaseTx(controlProgram []byte, height, txsFee uint64) (*types.Tx, error) {
	...
	if err := builder.AddOutput(types.NewOriginalTxOutput(*consensus.JTAAssetID, coinbaseValue, controlProgram, nil)); err != nil {
		return nil, err
	}
...
func CreateTxFromTx(baseTx *types.Tx, outputIndex uint64, outputAmount uint64, ctrlProgram []byte) (*types.Tx, error) {
	...
	output := types.NewOriginalTxOutput(*consensus.JTAAssetID, outputAmount, ctrlProgram, nil)
	builder := txbuilder.NewBuilder(time.Now())
...
func CreateRegisterContractTx(contract []byte) (*types.Tx, error) {
	txInput := types.NewSpendInput(nil, bc.NewHash([32]byte{0x01}), *consensus.JTAAssetID, 200000000, 1, []byte{0x51}, nil)
	...
	output := types.NewOriginalTxOutput(*consensus.JTAAssetID, 100000000, program, [][]byte{})
	builder := txbuilder.NewBuilder(time.Now())
	if err := builder.AddInput(txInput, &txbuilder.SigningInstruction{}); err != nil {
		return nil, err
	}
...
func CreateUseContractTx(hash []byte, arguments [][]byte, stateData [][]byte) (*types.Tx, error) {
	...
	txInput := types.NewSpendInput(nil, bc.NewHash([32]byte{0x01}), *consensus.JTAAssetID, 200000000, 1, program, stateData)
