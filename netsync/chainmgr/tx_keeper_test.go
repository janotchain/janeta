...
import (
	...
	"github.com/janotchain/janeta/test/mock"
	"github.com/janotchain/janeta/testcontrol"
)
...
func (m *mempool) GetTransactions() []*core.TxDesc {
	...
	for i := 0; i < txsNumber; i++ {
		txInput := types.NewSpendInput(nil, bc.NewHash([32]byte{0x01}), *consensus.JTAAssetID, uint64(i), 1, []byte{0x51}, [][]byte{})
		...
		tx := &types.Tx{
			...
			TxData: types.TxData{
				...
				Outputs: []*types.TxOutput{
					types.NewOriginalTxOutput(*consensus.JTAAssetID, uint64(i), []byte{0x6a}, [][]byte{}),
				},
				SerializedSize: 1000,
			},
			Tx: &bc.Tx{
